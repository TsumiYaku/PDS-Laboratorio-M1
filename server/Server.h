...
private:
    ...
    void parsePacket(std::pair<std::string, Message>); // parse the received communication and act accordingly

    // Pool queue methods
    void enqueuePacket(std::pair<std::string, Message>); // adds a packet to the pool's queue
    std::pair<std::string, Message> dequeuePacket(); // called by threads in the pool to consume communication
